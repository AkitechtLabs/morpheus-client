...
var MorpheusError = require('./error');
...
var INVALID_CREDENTIAL_ERROR = new MorpheusError(
  'Authentication Error: Your credentials are no longer valid. Please run ' + chalk.bold('morpheus login --reauth'),
  {exit: 1}
);
...
var _refreshAccessToken = function(refreshToken, authScopes) {
  ...
  return api.request('POST', '/oauth2/v3/token', {
    ...
    if (_.get(err, 'context.body.error') === 'invalid_scope') {
      throw new MorpheusError(
        'This command requires new authorization scopes not granted to your current session. Please run ' + chalk.bold('morpheus login --reauth') + '\n\n' +
        'For CI servers and headless environments, generate a new token with ' + chalk.bold('morpheus login:ci'),
        {exit: 1}
      );
    }
...
var logout = function(refreshToken) {
  ...
  return api.request('GET', '/o/oauth2/revoke', {
    ...
    throw new MorpheusError('Authentication Error.', {
      exit: 1
    });
  });
};
